...
class CatalogoViewsTests(TestCase):
    def setUp(self):
        ...
        for indice in range(10):
            Producto.objects.create(
                ...
                descripcion='Producto extra para paginacion.',
                precio='10.00',
                stock=2,
                categoria=self.categoria_regalos,
            )
    ...
    def test_catalogo_muestra_busqueda_por_texto(self):
        response = self.client.get(reverse('inicio'), {'q': 'sorpresa'})
    ...
    def test_inicio_muestra_bloques_de_servicio(self):
        ...
        self.assertContains(response, 'Como comprar')
        self.assertContains(response, 'Categorias favoritas')
    ...
    def test_catalogo_filtra_por_categoria(self):
        response = self.client.get(reverse('inicio'), {'categoria': self.categoria_flores.id})
    ...
    def test_detalle_producto_muestra_relacionados(self):
        response = self.client.get(reverse('detalle_producto', args=[self.producto_principal.id]))

        self.assertEqual(response.status_code, 200)
        self.assertContains(response, 'Caja sorpresa')
        self.assertContains(response, 'Kit cumpleanero')

    def test_catalogo_marca_productos_agotados(self):
        response = self.client.get(reverse('inicio'), {'q': 'ramo'})
